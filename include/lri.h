 ...
 public:
  ...
  template<typename MessageType>
      Subscriber<MessageType> Subscribe(
          ...
          Subscriber::SubscriberCallback<MessageType> callback);
...
/**
 * A subscriber listens to a particular topic and calls its callback function
 * when the topic gets a new event.
 */
...
/**
 * A monitor wraps a given message, turning it into a shared state.  It monitors
 * changes in the object, and sends state updates to subscribers when commit()
 * is called, or automatically at some maximum fixed rate.
 */
...
/**
 * A proxy wraps a given message, turning it into shared state.  It provides
 * const-access to message fields, and updates them automatically from the
 * publisher either when refresh is called or at some maximum fixed rate.
 */
